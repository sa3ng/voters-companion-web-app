<?php
    ...
    if($is_reply == 1){
        ...
        $stmt2 = $conn->prepare("INSERT INTO postsTBL(acc_id,message, is_reply, is_reply_of, likes) VALUES('" . $name["acc_id"] . "','". $message . "', 1 ,".$post_id.", 0)");
        $stmt2->execute();
        $stmt2->close();
    ...
    $stmt2 = $conn->prepare("INSERT INTO postsTBL(acc_id,header,message, is_reply, likes, approved) VALUES('" . $name["acc_id"] . "','". $header ."','". $message . "', 0, 0,1)");
    $stmt2->execute();
    $stmt2->close();
